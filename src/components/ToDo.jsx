    ...
    const alarm = () => {
        setTodos(todos.map(item => {
            if(item.created === created){
                let timeout = prompt("In how many seconds would you like to get an alert? (if empty, notification will be canceled)");
                if(timeout == "" || timeout == null) return item
                return{
                    ...item, notificationTime: new Date().getTime() + timeout * 1000
                }
            }
            return item
        }))
